...
def open_file(file_url, chatid):
    file_name, headers = urllib.request.urlretrieve(
        file_url, 'send2kindle_{}.pdf'.format(chatid))
...
#    msg.attach(MIMEApplication(open(files).read()))
    ...
    _msg = '{}\tSENT:\t{}\t{}\t{}'.format(datetime.datetime.now(), chatid,
                                          send_from, send_to)
    os.remove(files)
    _msg = ('File sent. Wait a few minutes and check on your device.'
            '\n\nSend a new command')
    bot.send_message(chatid, _msg, reply_markup=button)


def add_user(db, table, chatid):
    conn = sqlite3.connect(db)
    cursor = conn.cursor()
    sql = ("INSERT INTO {} (chatid, remetente, destinatario, criacao, usado)"
           "VALUES ('{}', '', '','{}', '{}')")
    aux = sql.format(table, chatid,  datetime.datetime.now(),
                     datetime.datetime.now())
    logger_info.info(datetime.datetime.now() + '\tUSER:\t' + str(chatid))
    cursor.execute(aux)
    conn.commit()
    conn.close()
...
def upd_user_last(db, table, chatid):
    conn = sqlite3.connect(db)
    cursor = conn.cursor()
    aux = 'UPDATE {} SET usado = {} WHERE chatid = {}'.format(
        table, '"{}"'.format(str(datetime.datetime.now())), chatid)
    cursor.execute(aux)
    conn.commit()
    conn.close()
...
def upd_user_email(db, table, chatid, email):
    ...
    if '@kindle.com' in email:
        aux = 'UPDATE {} SET destinatario = {} WHERE chatid = {}'.format(
            table, email, chatid)
    else:
        aux = 'UPDATE {} SET remetente = {} WHERE chatid = {}'.format(
            table, email, chatid)
    # print(aux)
    _msg = '{}\tUDP:\t{}\t{}'.format(str(datetime.datetime.now(), str(chatid),
                                     email))
    logger_info.info(_msg)
    cursor.execute(aux)
    conn.commit()
    conn.close()
...
if __name__ == '__main__':
    ...
    def start(message):
        ...
        if len(aux1) < 3 or len(aux2) < 3:
            ...
            bot.send_message(message.from_user.id, _msg, parse_mode='HTML',
                             reply_markup=button)
    ...
    def add_email(message):
        ...
        if ('@kindle.com' in message.text.lower() and
                validate_email(message.text.lower())):
            ...
            upd_user_email(db, table, message.from_user.id,
                           '"{}"'.format(str(message.text)))
            _msg = ('<b>Success</b>.\n To send a file to your Kindle,'
                    'click <b>Send file</b>.\nTo change your e-mail, click'
                    '<b>Set e-mail</b>.')
            bot.reply_to(message, _msg, parse_mode='HTML', reply_markup=button)
        else:
            _msg = ('<b>Error</b>.\n'
                    '{} is not valid.\n'
                    'Type your Kindle e-mail.')
            msg = bot.send_message(message.from_user.id,
                                   _msg.format(message.text)
                                   parse_mode='HTML')
            bot.register_next_step_handler(msg, add_email)
    ...
    def get_file(message):
        ...
        send_mail(str(message.from_user.id), data[2], data[3], 'Convert',
                  str(message.from_user.id), file_url)
